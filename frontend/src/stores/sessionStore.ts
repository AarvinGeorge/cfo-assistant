 ...
 * session ID, and workspace ID. Persisted to localStorage.
 *
 * Role in project:
 ...
 *   theme. Read by App.tsx (panel widths), main.tsx (theme), the chat
 *   store (session ID for LangGraph thread continuity), and axiosClient.ts
 *   (workspaceId for X-Workspace-ID header injection).
 *
 * Main parts:
 *   - SessionState: interface with themeMode, leftPanelOpen, rightPanelOpen,
 *     sessionId, workspaceId, and their toggle/setter actions.
